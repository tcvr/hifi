...
#if defined(Q_OS_MAC)
    static const unsigned long MAX_HEARTBEAT_AGE_USECS =
        600 * USECS_PER_SECOND;  // 10 mins with no checkin probably a deadlock, right now, on MAC
#else
...
Application::Application(int& argc, char** argv, QElapsedTimer& startupTimer, bool runningMarkerExisted) :
    ...
    _window->setWindowTitle("Tivoli Cloud VR 1.1");
    ...
    connect(&nodeList->getDomainHandler(), &DomainHandler::limitOfSilentDomainCheckInsReached, nodeList.data(),
    ...
    // so we defer the setup of the `scripting::Audio` class until this point
    {
        ...
        QSharedPointer<scripting::Audio> scriptingAudioSharedPointer =
            qSharedPointerDynamicCast<scripting::Audio>(DependencyManager::get<AudioScriptingInterface>());
        if (scriptingAudioSharedPointer) {
            connect(this, &Application::activeDisplayPluginChanged, scriptingAudioSharedPointer.data(),
                    &scripting::Audio::onContextChanged);
        }
    }
    ...
    connect(userInputMapper.data(), &UserInputMapper::actionEvent, [this](int action, float state) {
        ...
        QSharedPointer<scripting::Audio> audioScriptingInterface =
            qSharedPointerDynamicCast<scripting::Audio>(DependencyManager::get<AudioScriptingInterface>());
        {
    ...
    // CPM My version will not run an auto-update check.
    bool buildCanUpdate = false; // CPM this was =  BuildInfo::BUILD_TYPE == BuildInfo::BuildType::Stable || BuildInfo::BUILD_TYPE == BuildInfo::BuildType::Master;
    /*
...
// FIXME spacemouse code still needs cleanup
    ...
    render::entities::WebEntityRenderer::setReleaseWebSurfaceOperator([=](QSharedPointer<OffscreenQmlSurface>& webSurface,
                                                                          bool& cachedWebSurface,
...
void Application::cleanupBeforeQuit() {
    ...
    // DependencyManager::get<ScriptEngines>()->shutdownScripting(); // CPM !!!!!!! commented this out because it was making the shutdown time WAY too long !!!!!!
        ...
        if (_physicsEnabled) {
            {
                ...
                _physicsEngine->forEachDynamic([&](EntityDynamicPointer dynamic) { dynamic->prepareForPhysicsSimulation(); });
            }
    ...
    _graphicsEngine.editRenderArgs([this, deltaTime](AppRenderArgs& appRenderArgs) {
        ...
        appRenderArgs._sensorToWorld = getMyAvatar()->getSensorToWorldMatrix();
        {
            ...
            appRenderArgs._renderArgs = RenderArgs(_graphicsEngine.getGPUContext(), lodManager->getOctreeSizeScale(),
                                                   ...
                                                   RenderArgs::DEFAULT_RENDER_MODE, RenderArgs::MONO, RenderArgs::DEFERRED,
                                                   RenderArgs::RENDER_DEBUG_NONE);
...
void Application::addAssetToWorldAddEntity(QString filePath, QString mapping) {
    ...
    // bool grabbable = (Menu::getInstance()->isOptionChecked(MenuOption::CreateEntitiesGrabbable)); // CPM I do not want this feature.
    properties.setUserData(
        NOT_GRABBABLE_USER_DATA);  // CPM Just removing this ill-conceived feature. (grabbable ? GRABBABLE_USER_DATA : NOT_GRABBABLE_USER_DATA);
...
void Application::addAssetToWorldCheckModelSize() {
    ...
    while (item != _addAssetToWorldResizeList.end()) {
        ...
        if (doResize) {
            ...
            properties.setUserData(NOT_GRABBABLE_USER_DATA);  // CPM forcing it to not grabbagbe
...
void Application::takeSnapshot(bool notify, bool includeAnimated, float aspectRatio, const QString& filename) {
    addSnapshotOperator(std::make_tuple(
        [notify, includeAnimated, aspectRatio, filename](const QImage& snapshot) {
            qApp->postLambdaEvent([snapshot, notify, includeAnimated, aspectRatio, filename] {
                QString path =
                    DependencyManager::get<Snapshot>()
                        ->saveSnapshot(snapshot, filename, TestScriptingInterface::getInstance()->getTestResultsLocation());
                ...
                if (!includeAnimated) {
                    ...
                    SnapshotAnimated::saveSnapshotAnimated(path, aspectRatio,
                                                           DependencyManager::get<WindowScriptingInterface>());
                }
            });
        },
        aspectRatio, true));
...
void Application::takeSecondaryCameraSnapshot(const bool& notify, const QString& filename) {
    addSnapshotOperator(std::make_tuple(
        [notify, filename](const QImage& snapshot) {
...
void Application::setDisplayPlugin(DisplayPluginPointer newDisplayPlugin) {
    ...
    // Make the switch atomic from the perspective of other threads
    {
        ...
        RefreshRateManager::UXMode uxMode = isHmd ? RefreshRateManager::UXMode::VR : RefreshRateManager::UXMode::DESKTOP;
